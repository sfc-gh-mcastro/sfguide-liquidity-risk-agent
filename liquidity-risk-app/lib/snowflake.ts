...
function tomlConnectionConfig(conn: TomlConnection): snowflake.ConnectionOptions {
  ...
  // Expand ~ in privateKeyPath (SDK doesn't do this when options are passed manually)
  if (typeof normalized.privateKeyPath === "string" && normalized.privateKeyPath.startsWith("~")) {
    normalized.privateKeyPath = path.join(os.homedir(), normalized.privateKeyPath.slice(1))
  }
